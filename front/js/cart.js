...
postForm();
    ...
    //let total = 0;
   canapArray.produitLocalStorage.forEach(function (produit) {
      console.log(produit);
      // Création de l'article du produit
      let productArticle = document.createElement("article");
      productArticle.className = "cart__item";
      ...
      // Création de la div cart item content titlePrice
      let productItemContentTitlePrice = document.createElement("div");
      productItemContentTitlePrice.className = "cart__item__content__titlePrice";
  ...
  // Fonction pour récupérer les informations de plusieurs produits

  
  // Utilisation de la fonction pour récupérer les informations des produits
  getProductsData(productId)
    ...
    // si il y'as bien des article dans le locale storage je declare une boucle foreach pour placer les produit
...
// je fais une fonction pour calculer le total des prouduit
let total = 0;
for (let l = 0; l < produitLocalStorage.length; l++) {
total += produitLocalStorage[l].price * produitLocalStorage[l].quantity;
}
//insertion du total
document.getElementById('totalPrice').innerHTML = total;
...
function deleteProduct() {
    ...
    for (let i = 0; i < btn_supprimer.length; i++){
        btn_supprimer[i].addEventListener("click" , (event) => {
            ...
            let idDelete = produitLocalStorage[i].idProduct;
            let colorDelete = produitLocalStorage[i].color;

            // fonction pour verifier si il y as bien un element correspondant id et couleur
            produitLocalStorage = produitLocalStorage.filter( el => el.idProduct !== idDelete || el.color !== colorDelete );
...
// initation de la fonction posform pour le renseignement du formulaire
